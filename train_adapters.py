...
Config:
  hidden=2560, bottleneck=128, 32 adapters (one per OPT layer)
...
def find_resume_epoch():
    for epoch in range(NUM_EPOCHS, 0, -1):
        if (CHECKPOINT_DIR / f"epoch_{epoch}" / "adapter_weights.pt").exists():
            return epoch
    return 0
...
def save_checkpoint(adapters, hidden_size, epoch_losses):
    ...
    with open(CHECKPOINT_DIR / "train_meta.json", "w") as f:
        json.dump(_build_meta(adapters, hidden_size, epoch_losses), f, indent=2)
